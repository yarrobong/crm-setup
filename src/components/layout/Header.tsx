...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-surface/80 backdrop-blur-md border-b border-border-light transition-all">
      ...
      {/* Mobile Меню */}
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 top-20 bg-surface/95 backdrop-blur-lg z-40 overflow-y-auto px-4 py-6 border-t border-border-light pb-[calc(1.5rem+env(safe-area-inset-bottom))]">
          <nav className="flex flex-col gap-6 h-full">
            ...
            <div className="mt-auto pt-6 border-t border-border-light flex flex-col gap-4">
              <a href="tel:+70000000000" className="text-lg font-medium text-text-primary hover:text-primary">
                +7 (000) 000-00-00
              </a>
              <Button className="w-full justify-between" onClick={() => setIsMobileMenuOpen(false)}>
